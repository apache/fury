...
final class FuryByteReader extends ByteReader{

  FuryByteReader(Uint8List data, {int offset = 0, bool endian = true, int? length})
      : _offset = offset,
        _length = length ?? data.length,
        _bd = ByteData.sublistView(data, offset, length == null ? null : offset + length),
        super.internal(){
    // 检查的部分，ByteData的长度是否合法，其内部会检查的，我们这里不检查了
  }
  ...
  /// 要拿到Uint16List的view，必须要保证start offset是偶数, 只能copy了
  @override
  ...
  int readBytesAsInt64(int length){
    ...
    if (remaining >= 8) {
      ...
      if (endian == Endian.little) {
        ...
        res = length == 0 ? v : v & (0xffffffffffffffff >>> off); //TODO: 先和furyJava保持一致
        _offset += length;
      } else {
  ...
  // @override
  // int readVarUint32() {
  //   // TODO: now only support little-endian and relatively slow
