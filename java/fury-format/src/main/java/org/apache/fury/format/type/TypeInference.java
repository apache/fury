...
public class TypeInference {
  ...
  private static Field inferField(
      String name, TypeRef<?> typeRef, LinkedHashSet<Class<?>> seenTypeSet, CustomTypeHandler customTypes) {
    ...
    if (customEncoder != null) {
      ...
      Field f =
          inferField(DataTypes.ARRAY_ITEM_NAME, TypeUtils.getElementType(typeRef), seenTypeSet, customTypes);
      return DataTypes.arrayField(name, f);
  ...
  public static <T> void registerCustomCodec(final CustomTypeRegistration registration, final CustomCodec<T, ?> codec) {
    CustomTypeEncoderRegistry.registerCustomCodec(registration, codec);
  }

  public static <E, C extends Collection<E>> void registerCustomCollectionFactory(Class<?> iterableType, Class<E> elementType, CustomCollectionFactory<E, C> factory) {
