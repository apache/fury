...
uint32_t writeUCS2(v8::Isolate* isolate, uint8_t* buf, Local<String> str, int flags) {
...
static void serializeString(
  ...
  uint8_t* dst_data = reinterpret_cast<uint8_t*>(dst->Buffer()->GetBackingStore()->Data());
  ...
  if (is_one_byte && str->IsExternalOneByte()) {
    ...
    offset += str->Length();
      ...
      if (is_one_byte) {
          dst_data[offset++] = Encoding::LATIN1; // encoding
          offset += writeVarUint32(dst_data, offset, str->Length()); // length
          offset += str->WriteOneByte(isolate, dst_data + offset, 0, str->Length(), flags);
      } else {
...
v8::CFunction fast_serialize_string(
    v8::CFunction::Make(serializeStringFast));
