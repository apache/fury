...
// This code is based on the .NET implementation of the ArrayBufferWriter<T> class.

/// <summary>
/// Represents a heap-based, array-backed output sink into which <typeparam name="T"/> data can be written.
/// </summary>
internal struct PooledArrayBufferWriter<T> : IBufferWriter<T>, IDisposable
{
    ...
    /// <summary>
    /// Creates an instance of an <see cref="PooledArrayBufferWriter{T}"/>, in which data can be written to,
    ...
    public PooledArrayBufferWriter(int initialCapacity)
    {
        ...
        {
            ThrowHelper.ThrowArgumentException(paramName: nameof(initialCapacity));
        }
    ...
    public void Advance(int count)
    {
        if (count < 0)
        {
            ThrowHelper.ThrowArgumentException(paramName: nameof(count));
        }

        if (_index > _buffer.Length - count)
        {
            ThrowHelper.ThrowInvalidOperationException_PooledBufferWriterAdvancedTooFar(_buffer.Length);
        }
    ...
    {
        if (requiredFreeCapacity > FreeCapacity)
        {
            int currentLength = _buffer.Length;

            // Attempt to grow by the larger of the sizeHint and double the current size.
            int growBy = Math.Max(requiredFreeCapacity, currentLength);

            if (currentLength == 0)
            {
                growBy = Math.Max(growBy, StaticConfigs.BuiltInBufferDefaultCapacity);
            }
            ...
            if ((uint)newSize > int.MaxValue)
            {
                ...
                uint needed = (uint)(currentLength - FreeCapacity + requiredFreeCapacity);
                Debug.Assert(needed > currentLength);

                if (needed > ArrayMaxLength)
                {
                    ThrowHelper.ThrowOutOfMemoryException_BufferMaximumSizeExceeded(needed);
                }
    ...
    private void CheckAndResizeBuffer(int sizeHint)
    {
        if (sizeHint < 0)
        {
            ThrowHelper.ThrowArgumentException(paramName: nameof(sizeHint));
        }
