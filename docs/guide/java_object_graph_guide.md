...
| `basicTypesRefIgnored`            | `true`                                              | Influence on `Serializer#needToWriteRef` for basic types.                                                                                                   |
| `stringRefIgnored`                | `true`                                              | Influence on `Serializer#needToWriteRef` for `String`.                                                                                                       |
...
| `checkJdkClassSerializable`       | `true`                                              | Enables or disables checking of `Serializable` implementation for classes under `java.*`. If a class under `java.*` is not `Serializable`, Fury will throw an `UnsupportedOperationException`. |
